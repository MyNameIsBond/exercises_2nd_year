...
1) Insertion Sort:
Below you will find the code for insertion sort in the class 'All_sorts' (insertion).
basically this is, a slow sorting algorithm when it comes to sort a large list.
The steps are pretty simple.
In an unsorted list first check the 2nd element and if it's smaller then the previous(1st element) swap it.
otherwise move it to the next element of the list...
According to my comparisons_counter, this algorithm is going to compare n (depends on the len(list)) times.
...
If you run the insertion function the swaping will be printed as well as the steps.

2)selection_sort:
worst and best case scenario is O(n^2) or O(n**2)
the explaination is simple, goes through the list if by comparing finds the smallest which takes it and swap it in the
...
class All_sorts:
    ...
    def insertion(self,num_list):
        ...
        while bc < list_len:
            ...
            while sc > 0 and self.num_list[sc-1] > self.num_list[sc]:
                ...
                if sc > 0 and self.num_list[sc-1] > self.num_list[sc]: # so we can print weither is swapped or not.
            ...
            bc += 1
        print ('|{}| This is how many times this algth run'.format(counter))
    ...
    def selection_sort(self,num_list):
        ...
        print ('| {} | This is how many times this algth runs'.format(counter))
        return self.num_list
